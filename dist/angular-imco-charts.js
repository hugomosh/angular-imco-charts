      ...
      link: function postLink(scope, element, iAttrs, controller) {
        ...
        function build() {
          var margin = {
              ...
              bottom: 10,
              left: 0
            }, width = element.width(), height = element.height() - margin.top - margin.bottom;
          ...
          svg.attr('width', width + margin.left * 2 + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
          ...
          svg.append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
          var a = svg.append('g').attr('class', 'y axis').call(yAxis).attr('transform', 'translate(' + scope.config.yAxis.space + ',0)');
          if (scope.config.yAxis.visible) {
            a.append('text').attr('transform', 'rotate(-90)').attr('y', 6).attr('dy', '.71em').style('text-anchor', 'end').text(scope.config.yAxis.text);
          }
          ...
          }).on('mouseover', tip.show).on('mouseout', tip.hide).on('click', function (ele) {
            //console.debug(ele, this, 'queueue');
            if (scope.barClickFunction) {
              scope.barClickFunction({ ent: ele.entidad });
            }
          });
        ...
        function update(newValue) {
          ...
          svg.selectAll('.bar').data(data).classed('active', function (d) {
  ...
  function ($window) {
    ...
    var linkFunction = function link(scope, element, attrs) {
      ...
      var drawDout = function build() {
        ...
        result = g.append('path').datum({ endAngle: 0 }).attr('d', frontArc).attr('fill', function () {
